...
# lib mods
cookiejar.MozillaCookieJar.magic_re = (
    r".*"  # bypass the hardcoded "Netscape HTTP Cookie File" check
)
...
def request(
    url, args=None, byte_range=None, retries=HTTP_RETRY_COUNT, delay=HTTP_FETCH_DELAY
):
    ...
    try:
        ...
        if _retry:
            warn(
                "request failed: %s (%d retries left) -- will retry in %ds..."
...
def handle_game_updates(olditem, newitem):
    ...
    if olditem.long_title != newitem.long_title:
        try:
            info(
                '  -> long title has change "{}" -> "{}"'.format(
                    olditem.long_title, newitem.long_title
                )
            )
        ...
        if fetch_md5:
            if os.path.splitext(page.geturl())[1].lower() not in SKIP_MD5_FILE_EXT:
                ...
                try:
                    ...
                    warn(
                        "xml parsing error occurred trying to get md5 data for {}".format(
                            d.name
                        )
                    )
...
def process_argv(argv):
    p1 = argparse.ArgumentParser(
        description="%s (%s)" % (__appname__, __url__), add_help=False
    )
    ...
    g1.add_argument(
        "username", action="store", help="GOG username/email", nargs="?", default=None
    )
    g1.add_argument(
        "password", action="store", help="GOG password", nargs="?", default=None
    )

    g1 = sp1.add_parser(
        "update", help="Update locally saved game manifest from GOG server"
    )
    g1.add_argument(
        "-os",
        action="store",
        help="operating system(s)",
        nargs="*",
    ...
    g1.add_argument(
        "-lang",
        action="store",
        help="game language(s)",
        nargs="*",
        default=DEFAULT_LANG_LIST,
    )
    g2 = g1.add_mutually_exclusive_group()  # below are mutually exclusive
    g2.add_argument(
        "-skipknown", action="store_true", help="skip games already known by manifest"
    )
    g2.add_argument(
        "-updateonly", action="store_true", help="only games marked with the update tag"
    )
    g2.add_argument(
        "-id", action="store", help="id/dirname of a specific game to update"
    )
    ...
    g1.add_argument(
        "savedir",
        action="store",
        help="directory to save downloads to",
        nargs="?",
        default=".",
    )
    g1.add_argument(
        "-dryrun",
        action="store_true",
        help="display, but skip downloading of any files",
    )
    g1.add_argument(
        "-skipextras",
        action="store_true",
        help="skip downloading of any GOG extra files",
    )
    g1.add_argument(
    ...
    g1.add_argument(
        ...
        action="store",
        type=float,
        help="wait this long in hours before starting",
        default=0.0,
    )  # sleep in hr
    g1.add_argument(
        "-skipids",
        action="store",
        help="id[s] of the game[s] in the manifest to NOT download",
    )
    ...
    g1.add_argument(
        "src_dir", action="store", help="source directory to import games from"
    )
    g1.add_argument(
        "dest_dir", action="store", help="directory to copy and name imported files to"
    )
    ...
    g1.add_argument(
        "src_dir", action="store", help="source directory containing gog items"
    )
    g1.add_argument(
        "dest_dir", action="store", help="destination directory to backup files to"
    )

    g1 = sp1.add_parser(
        "verify",
        help="Scan your downloaded GOG files and verify their size, MD5, and zip integrity",
    )
    g1.add_argument(
        "gamedir",
        action="store",
        help="directory containing games to verify",
        nargs="?",
        default=".",
    )
    ...
    g1.add_argument(
        "-skipzip", action="store_true", help="do not perform zip integrity check"
    )
    g1.add_argument(
        "-delete",
        action="store_true",
    ...
    g1 = sp1.add_parser(
        "clean", help="Clean your games directory of files not known by manifest"
    )
    g1.add_argument(
        "cleandir",
        action="store",
    ...
    if args.cmd == "update":
        for lang in args.lang:  # validate the language
            if lang not in VALID_LANG_TYPES:
                error(
                    'error: specified language "%s" is not one of the valid languages %s'
        ...
        for os_type in args.os:  # validate the os type
            if os_type not in VALID_OS_TYPES:
                error(
                    'error: specified os "%s" is not one of the valid os types %s'
...
def cmd_login(user, passwd):
    ...
    if login_data["two_step_url"] is not None:
        ...
        with request(
            ...
            delay=0,
            args={
                "second_step_authentication[token][letter_1]": login_data[
                    "two_step_security_code"
                ][0],
                "second_step_authentication[token][letter_2]": login_data[
                    "two_step_security_code"
                ][1],
                "second_step_authentication[token][letter_3]": login_data[
                    "two_step_security_code"
                ][2],
                "second_step_authentication[token][letter_4]": login_data[
                    "two_step_security_code"
                ][3],
...
def cmd_update(os_list, lang_list, skipknown, updateonly, id):
    ...
    while not done:
        ...
        if i == 1:
            ...
            info(
                "fetching game product data (page %d / %d)..."
        ...
        url = (
            api_url
            + "?"
            + urlencode({"mediaType": media_type, "sortBy": "title", "page": str(i)})
        )
        ...
        with request(url, delay=0) as data_request:
            ...
            for item_json_data in json_data["products"]:
                ...
                if id:
                    if (
                        item.title == id or str(item.id) == id
    ...
    if not id and not updateonly and not skipknown:
        info(
            "found %d games !!%s" % (items_count, "!" * int(items_count / 100))
        )  # teehee
    ...
    for item in sorted(items, key=lambda item: item.title):
        ...
        i += 1
        info(
            "(%*d / %d) fetching game details for %s..."
        ...
        if h in md5_info:
            ...
            if os.path.isfile(dest_file):
                if h == hashfile(dest_file):
                    info(
                        "destination file already exists with the same md5 value.  skipping copy."
                    )
                    continue
    ...
    for item in sorted(items, key=lambda g: g.title):
        ...
        if not dryrun:
            with ConditionalWriter(
                os.path.join(item_homedir, INFO_FILENAME)
            ) as fd_info:
                ...
                if item.rating > 0:
                    fd_info.write(
                        "user rating.... {}%{}".format(item.rating * 2, os.linesep)
                    )
                ...
                if hasattr(item, "gog_messages") and item.gog_messages:
                    ...
                    for gog_msg in item.gog_messages:
                        fd_info.write(
                            "{0}{1}{0}".format(os.linesep, html2text(gog_msg).strip())
                        )
        ...
        if not dryrun:
            if item.serial != "":
                with ConditionalWriter(
                    os.path.join(item_homedir, SERIAL_FILENAME)
                ) as fd_serial:
        ...
        while not work.empty():
            ...
            try:
                ...
                with lock:
                    ...
                    if (
                        os.path.exists(path) and os.path.getsize(path) > sz
                ...
                with open_notrunc(path) as out:
                    ...
                    try:
                        ...
                        error(
                            "failed to download %s, byte_range=%s"
    ...
    for game in sorted(gamesdb, key=lambda g: g.title):
        ...
        for itm in game.downloads + game.extras:
            ...
            if os.path.isfile(src_file):
                ...
                if not os.path.exists(dest_file) or itm.size != os.path.getsize(
                    dest_file
                ):
                    info("copying to %s..." % dest_file)
    ...
    for cur_dir in sorted(os.listdir(cleandir)):
        ...
        if os.path.isdir(cur_fulldir) and cur_dir not in ORPHAN_DIR_EXCLUDE_LIST:
            if cur_dir not in items_by_title:
                ...
                expected_filenames = []
                for game_item in (
                    items_by_title[cur_dir].downloads + items_by_title[cur_dir].extras
                ):
                ...
                for cur_dir_file in os.listdir(cur_fulldir):
                    ...
                    if (
                        cur_dir_file not in expected_filenames
                        and cur_dir_file not in ORPHAN_FILE_EXCLUDE_LIST
                    ):
                        info(
                            "orphaning file '{}'".format(
...
def main(args):
    ...
    if args.cmd == "login":
        ...
        cmd_verify(
            args.gamedir, check_md5, check_filesize, check_zips, args.delete, args.id
        )
